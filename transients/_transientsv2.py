...
class DoveTransient():
    ...
    async def rho1(self, omega, hsthresh=0.5, renorm=True, weight=1):
        global delta_omega
        ...
        rho = lambda t: -self.Rabi1*((np.exp(-1J*(omega-1J*gamma0)*(t1-t0))-np.exp(-1J*(omega1-1J*gamma1)*(t1-t0)))/(1J*(omega1-omega-1J*(gamma1-gamma0))))\
        *np.exp(-1J*(omega1-1J*gamma1)*(t-t1))*(np.heaviside(t-t1, hsthresh))\
        +(1J/2)*(self.Rabi1*((np.exp(-1J*(omega-1J*gamma0)*(t-t0))-np.exp(-1J*(omega1-1J*gamma1)*(t-t0)))/(1J*(omega1-omega-1J*(gamma1-gamma0)))))\
